...
object LyricsSanitizer {
    private val NOISE_PATTERNS = listOf(
        Regex("""\s*[\(\[]\s*(?:Official\s+(?:Music\s+)?Video|Official\s+Audio|Lyric(?:s)?\s+Video|Audio\s+Oficial|Video\s+Oficial|Visualizer|Animated\s+Video|Performance\s+Video|Behind\s+The\s+Scenes|Live|Remix|(?:4K|HD|HQ)\s*(?:Video|Audio)?|feat\.?\s+[^\)\]]+|ft\.?\s+[^\)\]]+|prod\.?\s+(?:by\s+)?[^\)\]]+|MV|M/V|PV|Short\s+Ver(?:sion)?\.|Full\s+Ver(?:sion)?\.)\s*[\)\]]\s*""", RegexOption.IGNORE_CASE),
